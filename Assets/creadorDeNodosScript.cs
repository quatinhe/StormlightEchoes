...

{
            ...
            for (int i = 0; i < contadorNodos; i++) 
            {
                ...
                for (int l = 0; l < contadorNodos; l++)
                {
                    ...
                    if (resultado.posible)
                    {
                        ...
                        // Guardar el salto con direcci�n (velX) si quer�s
    ...
    void CrearNodoEnPlataformaDebajo(GameObject nodoOrigen, int derecha)
    {
        ...
        if (hit.collider != null && hit.collider.CompareTag("plataform"))
        {
            ...
            // Ajustamos un poco la altura para que no est� "pegado" al collider
...
private void imprimirCamino(int inicial, int fianl)
    ...
    (bool posible, float velX, float tiempo) EsSaltoPosibleYVelocidad(Vector2 origen, Vector2 destino)
    {
        ...
        // Resolver la ecuaci�n: y1 = y0 + v0y*t - 0.5*g*t^2
        ...
        if (discriminante < 0)
        {
            // No hay soluci�n real: el salto no llega a esa altura
            return (false, 0f, 0f);
        }
        ...
        // Elegimos la t que representa cuando el personaje ya est� cayendo (v < 0)
        float t = Mathf.Max(t1, t2);
        if (t < 0) return (false, 0f, 0f);

        // Verificamos que est� cayendo en ese momento (velocidad vertical < 0)
        float velY = v0y - g * t;
        if (velY > 0) return (false, 0f, 0f); // a�n est� subiendo
    ...
    bool HayObstaculoEntre(Vector2 origen, Vector2 vectorMaxHeight, Vector2 destino)
    {
        ...
        // RaycastAll desde origen a punto m�s alto
    ...
    // Funci�n auxiliar para detectar si hay obst�culos v�lidos
